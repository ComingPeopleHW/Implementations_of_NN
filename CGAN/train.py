...
def main():
    ...
    train_dataloader = get_dataloader(root='../datasets/{}/'.format(args.dataset_name) + args.dataset_name,
                                      transform=transform_, mode='train', batch_size=args.batch_size)
    val_dataloader = get_dataloader(root='../datasets/{}/'.format(args.dataset_name) + args.dataset_name,
                                    mode='val', transform=transform_, batch_size=args.batch_size)
    # Declare tensor type
    Tensor = torch.cuda.FloatTensor
    # if device == 'cuda' else torch.FloatTensor
    ...
    patch = (1, args.image_height // 2 ** 4, args.image_width // 2 ** 4)  # 先做阶乘
    # if device == 'cuda':
    ...
    if args.start_epoch != 0:
        ...
        generator.load_state_dict(
            torch.load("../saved_models/%s/generator_%d.pth" % (args.dataset_name, int(args.start_epoch))))
        discriminator.load_state_dict(
            torch.load("../saved_models/%s/discriminator_%d.pth" % (args.dataset_name, int(args.start_epoch))))
    else:
        # Initialize weights
    ...
    # Train model
    for epoch in range(int(args.start_epoch), N_EPOCHS):
        for batch_idx, data in enumerate(train_dataloader):
            ...
            # valid = torch.tensor(np.ones((real_A.size(0), *patch))).cuda()
            # fake = torch.tensor(np.zeros((real_A.size(0), *patch))).cuda()
            # valid = torch.tensor(np.ones((real_A.size(0), *patch)), requires_grad=False).cuda()
            # fake = torch.tensor(np.zeros((real_A.size(0), *patch)), requires_grad=False).cuda()
            ...
            save_image(img_sample, "../generated_images/{}/{}.png".format(args.dataset_name, batches_done),
                       nrow=5, normalize=True)

            if epoch % MODEL_INTERVAL == 0:
                # Save model checkpoints
                torch.save(generator.state_dict(),
                           "../saved_models/{}/generator_{}.pth".format(args.dataset_name, epoch))
            torch.save(discriminator.state_dict(),
                       "../saved_models/{}/discriminator_{}.pth".format(args.dataset_name, epoch))
